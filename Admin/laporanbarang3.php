...
?>
      ...
      <div class="card shadow mb-4">
                        <div class="card-header py-3">
                           <center> <h5 class="m-0 font-weight-bold text-dark">LAPORAN BARANG TERLARIS BERDASARKAN JENIS BARANG</h5></center>
                        </div>
                        </ol>
...
if(!empty($dt1) && !empty($dt2))
{
    ...
    where  (Tanggal_Faktur_Beli BETWEEN '$dt1' AND '$dt2') GROUP BY barang.Nama_Barang  order by Jumlah desc limit 10");
...
if(!empty($_POST['jenis']))
{
    ...
    where Nama_Jenis_Barang= '$jenis' GROUP BY barang.Nama_Barang  order by Jumlah desc limit 10");
}
if(!empty($_POST['jenis']) && !empty($dt1) && !empty($dt2))
{
    ...
    GROUP BY barang.Nama_Barang  order by Jumlah desc limit 10");
}
if(empty($_POST['jenis']) && empty($dt1) && empty($dt2))
{
    $ambil= mysqli_query ($koneksi,"SELECT Nama_Barang,sum(QTY_Barang) AS Jumlah FROM transaksi 
    ...
    JOIN kelompok_barang on barang.Kode_Kelompok_Barang=kelompok_barang.Kode_Kelompok_Barang
    JOIN jenis_barang On kelompok_barang.Kode_Jenis_Barang=jenis_barang.Kode_Jenis_Barang
    JOIN faktur_beli on faktur_rinci.ID_Faktur_Beli=faktur_beli.ID_Faktur_Beli GROUP BY barang.Nama_Barang order by Jumlah desc limit 10");
}
  }
?>
...
<?php if (isset($_POST['proses'])): ?>
        ...
        <span><button type="button" class="btn btn-success btn-rounded btn-fw" 
           onclick="location.href='index.php?halaman=laporanbarang3'" data-toggle="tooltip" data-placement="top" title="Kembali">
          Kembal
          </button>
          </span>
      <?php endif ?>
