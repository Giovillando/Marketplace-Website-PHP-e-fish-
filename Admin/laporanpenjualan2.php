...
<center><h3> Laporan Kelompok Barang Berdasarkan Penjualan </h3>
</center><hr>
  <form method="post">
    <div class="row">
      <div class="col-md-3">
      ...
      <div class="col-md-2">
        <div class="form-group">
          ...
          <select class="form-control" name="Kode_Kelompok_Barang">
            <option value="kelompok_barang">--Pilih Kelompok Barang--</option>
            <?php $ambil=$koneksi->query("SELECT * FROM kelompok_barang ");
                while($percabangbank=$ambil->fetch_assoc()){?>
                <option value="<?php echo $percabangbank['Nama_Kelompok_Barang']; ?>"><?php echo $percabangbank['Nama_Kelompok_Barang'];?>
                </option>
              <?php }?>
          </select>
        </div>
...
<?php
   $ambil=mysqli_query($koneksi,"SELECT *, sum(transaksi.QTY_Barang) AS total_transaksi FROM transaksi INNER JOIN barang ON transaksi.Kode_Barang = barang.Kode_Barang JOIN faktur_rinci ON transaksi.ID_Faktur_Rinci=faktur_rinci.ID_Faktur_Rinci JOIN kelompok_barang on barang.Kode_Kelompok_Barang = kelompok_barang.Kode_Kelompok_Barang  Join penjual on barang.ID_Penjual=penjual.ID_Penjual
    JOIN jenis_barang on kelompok_barang.Kode_Jenis_Barang=jenis_barang.Kode_Jenis_Barang WHERE Nama_Kelompok_Barang='$_POST[Kode_Kelompok_Barang]' AND faktur_rinci.Tanggal BETWEEN '$dt1' AND'$dt2' GROUP BY  penjual.ID_Penjual ORDER BY QTY_Barang DESC LIMIT 5");
  }
?>
      <?php $total=0; ?>
      <?php $nomor=1; ?>
